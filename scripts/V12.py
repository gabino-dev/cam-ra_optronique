...
class CANReader(QThread):
    ...
    update_lrf = Signal(int)  # nouvelle signal pour distance LRF
    ...
    def run(self):
        while self.running:
            ...
            if msg.arbitration_id == 0x105 and len(msg.data) >= 8:
                ...
                # Lecture de la distance LRF sur 2 octets (cm)
...
class CameraInterface(QWidget):
    def __init__(self):
        ...
        # Icône laser unicode (ou remplace par une icône QIcon PNG/SVG si tu veux)
        lrf_btn.setText("LRF \U0001F52B")  # 🔫 (pistolet laser) — change si tu veux autre chose
        ...
        self.can_thread.update_lrf.connect(self.on_lrf)  # Connecte la réception LRF
        ...
        self.send_unlock()  # Trame unlock au lancement
    ...
    def send_lrf(self):
        # Envoi de la trame LRF (laser range finder)
    ...
    def on_lrf(self, dist_cm):
        if dist_cm == 0:
            ...
            # arrondi à 2 décimales pour l’affichage en mètres
            dist_m = dist_cm 
