...
class CameraInterface(QWidget):
    ...
    def on_position(self, elev, bear):
        ...
        # --- Anti-tremblement servo ---
        if self.arduino and self.arduino.is_open:
            bearing_int = max(0, min(180, round(bear)))
            now = time.time()
            if (
                self.last_servo_angle is None or
                abs(bearing_int - self.last_servo_angle) > 2 or   # deadband de 2°
                (now - self.last_servo_time) > 0.12              # au moins 150 ms écoulées
            ):
                try:
                    message = f"{bearing_int}\n"
                    self.arduino.write(message.encode())
                    print(f"[DEBUG] Envoi vers Arduino : {bearing_int}")
                    self.last_servo_angle = bearing_int
